...
Q::~Q()
{
    delete[] u, v, p, up, vp, pp, un, vn, pn;
}
...
inline int Q::uIdx( int i, int j )
{
    return ( ( j ) * ( nmax + 1 ) + i );
}

// i=0:nmax, j=0:nmax+1, k=0:nmax
inline int Q::vIdx( int i, int j )
{
    return ( j * ( nmax ) + i );
}

// i=0:nmax, j=0:nmax, k=0:nmax+1
inline int Q::pIdx( int i, int j )
{
    return ( j * ( nmax ) + i );
}
...
void Q::getRes( double Re )
{
    ...
    for ( uint i = 1; i < longEnd; i++ )
    {
        ...
        for ( uint j = 1; j < shortEnd; j++ )
        {
            ...
            {
                Res[uIdx( i, j )]
                =c1*( ( pow( ( u[uIdx( i + 1, j )] + u[uIdx( i, j )] ), 2. ) - pow( u[uIdx( i, j )] + u[uIdx( i - 1, j )], 2. ) ) / dx * 0.25
                  + 0.25 / dy * ( ( v[vIdx( i, j + 1 )] + v[vIdx( i - 1, j + 1 )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j + 1 )] )
                                  - ( v[vIdx( i, j )] + v[vIdx( i - 1, j )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j - 1 )] ) ))

                  - 2. / Re / dx * ( ( u[uIdx( i + 1, j )] - u[uIdx( i, j )] ) / dx - ( u[uIdx( i, j )] - u[uIdx( i - 1, j )] ) / dx )
                  - 1. / Re / dy
                    * ( ( ( u[uIdx( i, j + 1 )] - u[uIdx( i, j )] ) / dy + ( v[vIdx( i, j + 1 )] - v[vIdx( i - 1, j + 1 )] ) / dx )
...
/*
                 ...
                 +0.25*(v[vIdx(i,j)]+v[vIdx(i,j+1)]+ v[vIdx(i-1,j)]+v[vIdx(i-1,j+1)])*(u[uIdx(i,j+1)]-u[uIdx(i,j-1)])/2./dy);
*/
    ...
    for ( uint i = 1; i < shortEnd; i++ )
    {
        ...
        for ( uint j = 1; j < longEnd; j++ )
        {
...

            {
                ...
                Res[sizeS + vIdx( i, j )]
                =c1*( ( pow( ( v[vIdx( i, j + 1 )] + v[vIdx( i, j )] ), 2. ) - pow( v[vIdx( i, j )] + v[vIdx( i, j - 1 )], 2. ) ) / dy * 0.25
                  + 0.25 / dx * ( ( v[vIdx( i + 1, j )] + v[vIdx( i, j )] ) * ( u[uIdx( i + 1, j )] + u[uIdx( i + 1, j - 1 )] )
                                  - ( v[vIdx( i, j )] + v[vIdx( i - 1, j )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j - 1 )] ) ))
                  - 2. / Re / dy * ( ( v[vIdx( i, j + 1 )] - v[vIdx( i, j )] ) / dy - ( v[vIdx( i, j )] - v[vIdx( i, j - 1 )] ) / dy )
                  - 1. / Re / dx
                    * ( ( ( u[uIdx( i + 1, j )] - u[uIdx( i + 1, j - 1 )] ) / dy + ( v[vIdx( i + 1, j )] - v[vIdx( i, j )] ) / dx )
                        - ( ( u[uIdx( i, j )] - u[uIdx( i, j - 1 )] ) / dy + ( v[vIdx( i, j )] - v[vIdx( i - 1, j )] ) / dx ) )

// skew symmetric formulation 
/* dumb version
                 +c2*(v[vIdx(i,j)]*0.5/dy*(v[vIdx(i,j+1)]-v[vIdx(i,j)]+v[vIdx(i,j)]-v[vIdx(i,j-1)]) 
                 +0.25*(u[uIdx(i,j)]+u[uIdx(i+1,j)]+ u[uIdx(i,j-1)]+u[uIdx(i+1,j-1)])*(v[vIdx(i+1,j)]-v[uIdx(i-1,j)])/2./dx);
*/
        ...
        for ( uint i = 1; i < shortEnd; i++ )
        {
            for ( uint j = 1; j < shortEnd; j++ )
            {
                //                    for ( uint k = 1; k < shortEnd; k++ )
                {
                    ...
                    val = -( ( un[uIdx( i + 1, j )] - un[uIdx( i, j )] ) / dx + ( vn[vIdx( i, j + 1 )] - vn[vIdx( i, j )] ) / dy ) / dt
    ...
    for ( uint l = 0; l < 12; l++ )
    {
        ...
        for ( uint i = 1; i < shortEnd; i++ )
        {
            for ( uint j = 1; j < shortEnd; j++ )
            {
                //                    for ( uint k = 1; k < shortEnd; k++ )
                {
                    val = ( ( un[uIdx( i + 1, j )] / dt - ( pn[pIdx( i + 1, j )] - pn[pIdx( i, j )] ) / dx )
                            + -( un[uIdx( i, j )] / dt - ( pn[pIdx( i, j )] - pn[pIdx( i - 1, j )] ) / dx ) ) / dx
                          + +( ( vn[vIdx( i, j + 1 )] / dt - ( pn[pIdx( i, j + 1 )] - pn[pIdx( i, j )] ) / dy )
                               + -( vn[vIdx( i, j )] / dt - ( pn[pIdx( i, j )] - pn[pIdx( i, j - 1 )] ) / dy ) ) / dy;
...
#endif
void Q::Struct_2D_Ghost( double Xa, double Xb, double Ya, double Yb, double **X, double **Y )
...
void Q::debug( double Xa, double Ya )
{
    ...
    Uxy( Xa, Ya, i, j, xy );
    cout << "U(i,j) coord " << xy[0] << " " << xy[1] << " value " << u[uIdx( i, j )] << endl;
    Uxy( Xa, Ya, i + 1, j, xy );
    cout << "U(i+1,j) coord " << xy[0] << " " << xy[1] << " value " << u[uIdx( i + 1, j )] << endl;
    Uxy( Xa, Ya, i, j + 1, xy );
    cout << "U(i,j+1) coord " << xy[0] << " " << xy[1] << " value " << u[uIdx( i, j + 1 )] << endl;
    Uxy( Xa, Ya, i + 1, j + 1, xy );
    cout << "U(i+1,j+1) coord " << xy[0] << " " << xy[1] << " value " << u[uIdx( i + 1, j + 1 )] << endl;
    Uxy( Xa, Ya, i - 1, j + 1, xy );
    cout << "U(i-1,j+1) coord " << xy[0] << " " << xy[1] << " value " << u[uIdx( i - 1, j + 1 )] << endl;

    cout << "**********************************************************" << endl;
    Vxy( Xa, Ya, i, j, xy );
    cout << "V(i,j) coord " << xy[0] << " " << xy[1] << " value " << v[vIdx( i, j )] << endl;
    Vxy( Xa, Ya, i - 1, j, xy );
    cout << "V(i-1,j) coord " << xy[0] << " " << xy[1] << " value " << v[vIdx( i - 1, j )] << endl;
    Vxy( Xa, Ya, i, j + 1, xy );
    cout << "V(i,j+1) coord " << xy[0] << " " << xy[1] << " value " << v[vIdx( i, j + 1 )] << endl;
    Vxy( Xa, Ya, i - 1, j + 1, xy );
    cout << "V(i-1,j+1) coord " << xy[0] << " " << xy[1] << " value " << v[vIdx( i - 1, j + 1 )] << endl;
    Vxy( Xa, Ya, i - 1, j + 1, xy );
    cout << "V(i-1,j+1) coord " << xy[0] << " " << xy[1] << " value " << v[vIdx( i - 1, j + 1 )] << endl;
    ...
    // duu_dx= -0.25*(pow( ( u[uIdx( i + 1, j )] + u[uIdx( i, j )] ), 2. ) - pow( u[uIdx( i, j )] + u[uIdx( i - 1, j )], 2. ) ) / dx;
    duu_dx = pow( ( u[uIdx( i + 1, j )] + u[uIdx( i, j )] ), 2. ) - pow( u[uIdx( i, j )] + u[uIdx( i - 1, j )], 2. );

    duv_dy = 0.25 / dy * ( ( v[vIdx( i, j + 1 )] + v[vIdx( i - 1, j + 1 )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j + 1 )] )
                           - ( v[vIdx( i, j )] + v[vIdx( i - 1, j )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j - 1 )] ) );
    ...
    double valx =
    -( pow( ( u[uIdx( i + 1, j )] + u[uIdx( i, j )] ), 2. ) - pow( u[uIdx( i, j )] + u[uIdx( i - 1, j )], 2. ) ) / dx * 0.25
    - 0.25 / dy * ( ( v[vIdx( i, j + 1 )] + v[vIdx( i - 1, j + 1 )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j + 1 )] )
                    - ( v[vIdx( i, j )] + v[vIdx( i - 1, j )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j - 1 )] ) )
    - 1. / dx * ( p[pIdx( i, j )] - p[pIdx( i - 1, j )] )
    + 2. / Re / dx * ( ( u[uIdx( i + 1, j )] - u[uIdx( i, j )] ) / dx - ( u[uIdx( i, j )] - u[uIdx( i - 1, j )] ) / dx )
    + 1. / Re / dy * ( ( ( u[uIdx( i, j + 1 )] - u[uIdx( i, j )] ) / dy + ( v[vIdx( i, j + 1 )] - v[vIdx( i - 1, j + 1 )] ) / dx )
                       - ( ( u[uIdx( i, j )] - u[uIdx( i, j - 1 )] ) / dy + ( v[vIdx( i, j )] - v[vIdx( i - 1, j )] ) / dx ) );

    double valy =
    -( pow( ( v[vIdx( i, j + 1 )] + v[vIdx( i, j )] ), 2. ) - pow( v[vIdx( i, j )] + v[vIdx( i, j - 1 )], 2. ) ) / dy * 0.25
    - 0.25 / dx * ( ( v[vIdx( i + 1, j )] + v[vIdx( i, j )] ) * ( u[uIdx( i + 1, j )] + u[uIdx( i + 1, j - 1 )] )
                    - ( v[vIdx( i, j )] + v[vIdx( i - 1, j )] ) * ( u[uIdx( i, j )] + u[uIdx( i, j - 1 )] ) )
    - 1. / dy * ( p[pIdx( i, j )] - p[pIdx( i, j - 1 )] )
    + 2. / Re / dy * ( ( v[vIdx( i, j + 1 )] - v[vIdx( i, j )] ) / dy - ( v[vIdx( i, j )] - v[vIdx( i, j - 1 )] ) / dy )
    + 1. / Re / dx * ( ( ( u[uIdx( i + 1, j )] - u[uIdx( i + 1, j - 1 )] ) / dy + ( v[vIdx( i + 1, j )] - v[vIdx( i, j )] ) / dx )
                       - ( ( u[uIdx( i, j )] - u[uIdx( i, j - 1 )] ) / dy + ( v[vIdx( i, j )] - v[vIdx( i - 1, j )] ) / dx ) );
    ...
    cout<<"compare "<<u[i]<<" "<<un[i]<<" "<<up[i]<<endl;
